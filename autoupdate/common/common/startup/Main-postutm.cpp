...
int main(int argc, char* argv[])
{
	...
	::CreateMutex(NULL, FALSE, "twautoupdate"); /*创建一个互斥体*/
...
// 		char sTmp[1024];
// 		GetPrivateProfileString("system", "title", "No name", sTmp, 1024, "./system.conf");
// 		std::string consoleTitle = sTmp;
// 		consoleTitle += " - ";
// 
// 		consoleTitle += "[";
// 		consoleTitle += pcname;
...
// 		consoleTitle += cmd.serviceVersion();
		std::string consoleTitle = cmd.serviceVersion();
		...
		// 隐藏窗口
// 		HWND hWnd;	// 此方法，在多次启动后窗口会重新显示出来
// 		hWnd=::FindWindow(NULL,consoleTitle.c_str()); 
